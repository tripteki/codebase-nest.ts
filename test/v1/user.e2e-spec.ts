...
describe ("User (e2e)", (): void =>
{
    ...
    describe ("User Auth", (): void =>
    {
        ...
        it ("auth reset-password (POST)", async (): Promise<void> =>
        {
            ...
            const frontendUrl: string = configService.get<string> ("app.frontendUrl");
            const frontendUrlObj = new URL (frontendUrl);
            const testHost = `${frontendUrlObj.hostname}${frontendUrlObj.port ? ':' + frontendUrlObj.port : ''}`;
            ...
            const signedUrl = urlHelper.ref ().sign (`${frontendUrl}/auth/reset-password/user@mail.com`);
            ...
            const response = await request (app.getHttpServer ())
            .post (`/v1/auth/reset-password/user@mail.com?signed=${signedToken}`)
            .set ("Host", testHost)
            .send ({
        ...
        it ("auth verify-email (POST)", async (): Promise<void> =>
        {
            ...
            const signedUrl = urlHelper.ref ().sign (`${frontendUrl}/auth/verify-email/user@mail.com`);
            ...
            const response = await request (app.getHttpServer ())
            .post (`/v1/auth/verify-email/user@mail.com?signed=${verifyToken}`)
            .set ("Host", testHost)
            .set ("Authorization", `Bearer ${token}`)
            .expect (200);
...
test-import-2,import2@test.com,password123`;
    ...
    describe ("User Profile", (): void =>
    {
        ...
        it ("users me show (GET)", async (): Promise<void> =>
        {
            const response = await request (app.getHttpServer ())
            .get ("/v1/users/me")
            .set ("Authorization", `Bearer ${token}`)
        ...
        it ("users me update (PUT)", async (): Promise<void> =>
        {
            const prismaPostgreService: DatabasePrismaPostgreDriver = app.get (DatabasePrismaPostgreDriver);
            const user = await prismaPostgreService.user.findUnique ({
                where: { email: "user@mail.com", },
            ...
            expect (response.body.profile?.interests).toEqual (expect.arrayContaining ([ "Design", "Product", ]));
        });

        it ("users me update multipart with avatar (POST)", async (): Promise<void> =>
        {
            const prismaPostgreService: DatabasePrismaPostgreDriver = app.get (DatabasePrismaPostgreDriver);
            const user = await prismaPostgreService.user.findUnique ({
                where: { email: "user@mail.com", },
            ...
            .post ("/v1/users/me")
            .set ("Authorization", `Bearer ${token}`)
            .field ("name", user?.name ?? "test-user")
            .field ("email", "user@mail.com")
            .field ("full_name", "Avatar User")
            .attach ("avatar", pngAvatar, {
                filename: "avatar.png",
                contentType: "image/png",
            })
            .expect (200);

            expect (response.body.profile?.full_name).toBe ("Avatar User");
            expect (response.body.profile?.avatar).toMatch (/^avatars\//);
            expect (response.body.profile?.avatar_url).toContain ("/storage/avatars/");
        });

        it ("users me update multipart rejects invalid avatar (POST)", async (): Promise<void> =>
        {
            const prismaPostgreService: DatabasePrismaPostgreDriver = app.get (DatabasePrismaPostgreDriver);
            const user = await prismaPostgreService.user.findUnique ({
                where: { email: "user@mail.com", },
            ...
            const textFile = Buffer.from ("not-an-image", "utf-8");
        ...
        it ("users me interests (GET)", async (): Promise<void> =>
        {
            const response = await request (app.getHttpServer ())
        ...
        it ("users me accesses (GET)", async (): Promise<void> =>
        {
            const response = await request (app.getHttpServer ())
