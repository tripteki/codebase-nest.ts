                ...
                try {
                    ...
                    const existingUser = await this.prismaPostgreService.user.findFirst ({
                        where: {
                            deleted_at: null,
                            OR: [
                                { email: row[this.i18nService.t ("_module_v1_user.import.column.email")] || row.email, },
                                { name: row[this.i18nService.t ("_module_v1_user.import.column.name")] || row.name, },
                            ],
                        },
                    });
                    ...
                    const hashedPassword = await this.hasherHelper.hash (row[this.i18nService.t ("_module_v1_user.import.column.password")] || "12345678");
                    ...
                    await this.prismaPostgreService.user.create ({
                        data: {
                            ...
                            email_verified_at: row[this.i18nService.t ("_module_v1_user.import.column.email_verified")] === this.i18nService.t ("_module_v1_user.common.yes") ? this.dateTimeHelper.now () : null,
