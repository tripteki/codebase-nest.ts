    ...
    public async handle (job: Job<{ userId: string; type?: string; }>): Promise<void>
    {
        ...
        try {
            ...
            const data = users.map ((user) => ({
                ...
                [this.i18nService.t ("_module_v1_user.export.column.id")]: user.id,
                [this.i18nService.t ("_module_v1_user.export.column.name")]: user.name,
                [this.i18nService.t ("_module_v1_user.export.column.email")]: user.email,
                [this.i18nService.t ("_module_v1_user.export.column.email_verified")]: user.email_verified_at ? this.i18nService.t ("_module_v1_user.common.yes") : this.i18nService.t ("_module_v1_user.common.no"),
                [this.i18nService.t ("_module_v1_user.export.column.created_at")]: user.created_at.toISOString (),
                [this.i18nService.t ("_module_v1_user.export.column.updated_at")]: user.updated_at.toISOString (),
            }));

            const filename = `users_export_${Date.now ()}.${type}`;
            const sheetName = this.i18nService.t ("_module_v1_user.export.sheet_name");
