...
import { AuthIdentifierTransformerDto, AuthTransformerDto, UserTransformerDto, } from "src/v1/api/user/dtos/user.transformer.dto";
...
export class UserAuthService extends AppService
{
    ...
    /**
     * @param {AuthIdentifierTransformerDto} userId
     * @returns {Promise<AuthTransformerDto>}
     */
    protected async attempt (
        { userId, }: AuthIdentifierTransformerDto
    ): Promise<AuthTransformerDto>
    {
        const payloadDto: { sub: string; } = {
            sub: userId,
        };
        const authDto: AuthTransformerDto = plainToClass (AuthTransformerDto, {
    ...
    ): Promise<AuthTransformerDto | string>
    {
        try {
            ...
            const authDto: AuthTransformerDto = await this.attempt ({ userId: userDto.id, });
    ...
    ): Promise<AuthTransformerDto | string>
    {
        try {
            ...
            const authDto: AuthTransformerDto = await this.attempt ({ userId, });
